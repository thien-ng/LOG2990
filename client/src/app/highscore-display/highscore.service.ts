...
export class HighscoreService {
  ...
  public getHighscore(id: number): void {
    this.httpClient.get(Constants.BASE_URL + Constants.HIGHSCORE_PATH + id).subscribe((data: HighscoreMessage) => {
      this.highscoreUpdated.next(data);
    });
  }

  public resetHighscore(id: number): void {
    this.httpClient.get(Constants.BASE_URL + Constants.HIGHSCORE_PATH + Constants.RESET_PATH + id).subscribe(() => {
